...
在主线程上运行 Blitz 渲染引擎，接收 JS 线程发来的 `DomMsg`，将其应用到 `blitz-dom` 的 `BaseDocument`，并将窗口事件（点击、键盘等）通过 `Event` 反馈给 JS 线程。
...
| `src/event.rs` | `convert_event()`：winit `WindowEvent` → `Event`，投递到 `event_tx` |
...
window.handle_winit_event(event)     ← 由 blitz-shell 内部处理点击命中测试
    ...
    │
    └─ 用户事件（click/key）→ convert_event() → Event
                                                   │
                                          event_tx.send(Event)
                                                   │
                                          JS 线程 event_rx.recv()
...
| `event_tx: Sender<Event>` | **crossbeam-channel** | Blitz → JS，用户事件回传 |
| `BlitzApplication::event_queue` | `std::sync::mpsc`（blitz-shell 内部） | winit 事件循环唤醒，**不可更改**，仅用于触发主线程 wake up |

webAtom 自己的消息（`DomMsg` / `Event`）**全部走 crossbeam-channel**。`std::sync::mpsc` 只作为 winit 事件循环的唤醒触发器，不传递业务数据。

### Event 回传机制

Blitz 主线程命中测试后调用 `event_tx.send(Event)`（crossbeam，无锁，`Send + Sync`）。JS 侧不订阅推送，而是在事件循环每个 tick 的 idle 阶段主动轮询：
