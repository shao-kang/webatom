...
| 文件 | 职责 |
|---|---|
| `event.rs` | `MacroTask` 类型定义（跨线程宏任务闭包） |
...
| 类型 | 归属 | 原因 |
|---|---|---|
| `task_tx` / `vsync_tx` / `keepalive_count` | `EventLoopHandle`（跨线程） | 需要被 tokio task / 渲染线程写入 |
...
| **Tokio（外部）** | `tokio::spawn` | 产生 `MacroTask` 闭包推入 `task_tx`，不直接执行 JS | `setTimeout`、`fetch`、`WebSocket` |
    ...
    loop {
        ...
        loop {
            ...
            match self.task_rx.try_recv() {
                Ok(task) => self.process_event(context, task).await?,
                Err(_) => break,
            }
        }

        // ③ Idle 提前检查（task_rx 空时立即处理）
        if self.task_rx.is_empty() {
            self.idle_scheduler.flush_timed_out(context).await?;
            if self.task_rx.is_empty() && !self.idle_scheduler.is_empty() {
        ...
        if !self.render_thread_dead {
            tokio::select! {
                ...
                Some(task) = self.task_rx.recv() => {
                    self.process_event(context, task).await?;
                }
...
## MacroTask（闭包式宏任务，Extension 自定义）

```rust
// event.rs
pub type MacroTask = Box<dyn for<'js> FnOnce(Ctx<'js>) -> rquickjs::Result<()> + Send + 'static>;
```
...
- `EventLoopHandle` 不需要注册为 userdata（Extension 通过 `HostBridge.runtime.keepalive` 和 `HostBridge.io.task_tx` 访问）
...
async fn should_exit(&self, context: &AsyncContext) -> rquickjs::Result<bool> {
    ...
    if !self.task_rx.is_empty()               { return Ok(false); }
...
pub struct EventLoopHandle {
    ...
    pub task_tx: mpsc::Sender<MacroTask>,              // bounded 1024，Extension 推宏任务
...
## Timer 实现（Extension 自治，闭包封装）

Extension 直接构造闭包推入 `task_tx`：
    ...
    loop {
        tokio::select! {
            _ = tokio::time::sleep_until(next_fire.into()) => {
                let func = persistent.clone();   // interval 需复用 func，clone Persistent
                let task: MacroTask = Box::new(move |ctx| {
...
tokio Thread Pool
  └── task_tx.send(MacroTask).await（bounded 1024，back-pressure）
      嵌套 select：send + cancel_rx 竞争，keepalive 保证 Runtime 不提前退出
      Extension 自行封装闭包，无需声明 RuntimeEvent 类型
...
| MacroTask 闭包化 | Extension 推 `Box<dyn FnOnce(Ctx)>`；加新 Extension 不改 EventLoop |
